...
namespace CepProject
{
    public partial class Form1 : Form
    {
        ...
        private double[] Cepstum(double[] spectral)
        {
            double[] cepstum = new double[400];
            ...
            for (int i = 0; i < 400; i++)
            {
                ...
                for (int j = 0; j < 400; j++)
                {
                    P += Math.Cos(2 * Math.PI * i * j / 400) * Math.Log(spectral[j]);
                    Q += Math.Sin(2 * Math.PI * i * j / 400) * Math.Log(spectral[j]);
                }

                cepstum[i] = Math.Pow(P, 2) + Math.Pow(Q, 2);
        ...
        private void CreateWindow_Click(object sender, EventArgs e)
        {
            ...
            double[] s = Apply_FFT(signalQ);
            double[] spectral = Cepstum(s);
            ...
            for (int i = 1; i < spectral.Length - 1; i++)
            {
